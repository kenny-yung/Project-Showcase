...
void femSolver::explicitSolver()
{
    ...
    for (int iter=0; iter<nIter; iter++)
    {
        ...
        #pragma omp parallel
        {
            ...
            //#pragma omp for private(elem, M, F, K, TL, i, MTnewL) reduction(+: MTnew[0:nn]) schedule(dynamic,128)
            ...
            // Uncomment one of the following lines to switch between different scheduling strategies.
            #pragma omp for private(pNode, massTmp, MT, Tnew) reduction(+:partialL2error) schedule(static,128)
            //#pragma omp for private(pNode, massTmp, MT, Tnew) reduction(+:partialL2error) schedule(dynamic,128)
