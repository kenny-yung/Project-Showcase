...
def run_memory_test(sess, loss, outputs, labels, it_test_series_init, sz1):
    ...
    :param sz1: Size of the sequence to remember
    :return:
    """
    #####Start Subtask 2f#####
    outputs = [tf.expand_dims(o, axis=1) for o in outputs[-sz1:]]
    outputs = tf.concat(outputs, axis=1)
    acc, acc_update_op = tf.metrics.accuracy(labels[:, -sz1:], tf.argmax(outputs, axis=2), name='test')
